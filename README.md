...
3. **Configure Appwrite Backend**
   - Update `backend/appwrite.js` with your Appwrite credentials:
     - `endpoint`: Your Appwrite endpoint
     - `projectId`: Your Appwrite project ID
     - `databaseId`: Your Appwrite database ID
   - Ensure the following tables exist in your Appwrite database:
     - `users`: User accounts (name, email, password, isProfessor)
     - `courses`: Course information (code, name, schedule, location, professorId)
...
4. **Configure Google Places API**
   - Add your Google Places API key to `utils/googlePlaces.js`
